...
format long
for count = 1: columns
    CCPP(:,count) = (CCPP(:,count) - min(CCPP(:,count))) / (max(CCPP(:,count)) - min(CCPP(:,count)));
end
format short
...
%% Create the model %%
%~ 3 Features, with Gaussian Membership Functions and Polynomial Output ~%
Model = genfis1(D_trn, 3, 'gbellmf','linear'); %
...
sgtitle('Model 4: Membership Functions of Initial Model')
subplot(2,2,1);
...
sgtitle('Model 4: Membership Functions of Trained Model')
subplot(2,2,1);
...
sgtitle('Learning Curve of Model 4')
...
sgtitle('Model 4: Prediction Error' )
subplot(1,2,1)
plot(Model_Output(1:100)); hold on;
plot(D_chk(1:100,5),'r'); legend(' Model Output ', ' Real Output ');
